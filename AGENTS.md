...
| `mcp.ts` | MCP bridge: derives the remote MCP tool list from `CHAT_TOOLS` (+ MCP-only `get_profile`), dispatches via `executeTool`, and enforces the fail-closed `MCP_AUTH_TOKEN` bearer auth. Served by `app/api/mcp/route.ts` so Claude on the user's subscription (claude.ai custom connector / Claude Code) can query the diary. Read-only by design — see the do-not-regress rule in CLAUDE.md. |
