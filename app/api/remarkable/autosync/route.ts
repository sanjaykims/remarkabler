...
// kept fresh by the maintenance sweep). Enabling kicks a sync attempt so the
// user sees results without waiting for the next sweep.
export async function POST(req: NextRequest) {
  ...
  if (body.enabled === true) {
    void maybeSyncRemarkable().catch(() => {});
  }
