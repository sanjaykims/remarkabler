...
describe("app authentication route", () => {
  it("clears passcode failures after a successful WebAuthn login", async () => {
    ...
    // Challenges are bound server-side now, so a value the server never issued
    // is refused before the lockout behaviour under test is reached.
    auth.rememberChallenge("challenge", "login");
