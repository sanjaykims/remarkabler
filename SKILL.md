...
| `claude.ts` | Every Anthropic SDK call. Model resolution chain: in-app setting → env var → built-in default, per role (`modelMain` / `modelChat` / `modelChatFallback` / `modelChatMemory`). |
...
| `/chat` | Conversational chat with Claude over the profile. Image/PDF attachments. Voice in/out (Android). Clear button (= archive + chat-memory extraction). |
