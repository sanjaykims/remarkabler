...
function candidates(kind: Kind): Array<{ norm: string; name: string }> {
  return db()
    ...
    .all(kind, DISCIPLINE_ID, CONVERSATIONS_NOTEBOOK_ID, REFLECTIONS_NOTEBOOK_ID, DECISIONS_NOTEBOOK_ID) as Array<{ norm: string; name: string }>;
}
...
function mentions(
  ...
  return db()
    ...
    .all(kind, norm, DISCIPLINE_ID, CONVERSATIONS_NOTEBOOK_ID, REFLECTIONS_NOTEBOOK_ID, DECISIONS_NOTEBOOK_ID) as Array<{
    page_id: string;
    date: string | null;
    text: string;
  }>;
