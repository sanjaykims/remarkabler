...
export async function chatOverNotes(opts: {
  ...
  const staticGuidance = [
    ...
    "",
    "Match your answer to what they asked. When it's a factual or look-up",
    "question (\"where was I\", \"what did I do on X\", a list, a timeline),",
    "lead with the facts and stop there. Two rules for those:",
    "- Never fill a gap with a guess dressed as fact. If your notes or",
    "  memory say something was PLANNED or only POSSIBLE, don't narrate it",
    "  as though it happened (\"the Samsung trip got moved\", \"your Suwon",
    "  visit\") — report only what the data actually shows, and where it's",
    "  silent, say so plainly. Don't invent a reason for the gap AND don't",
    "  wave it away (\"probably nothing\", \"not missing data on my end\",",
    "  \"you were likely just in transit\") — a gap is just a gap. Naming it",
    "  once is enough; you don't need to reassure them about it.",
    "- Don't turn a factual request into an interview. A location list",
    "  doesn't need \"how are you holding up?\", \"did the visit happen?\", or",
    "  a remark on how little they slept. Answer what was asked; save the",
    "  check-ins for when they're actually reflecting with you.",
    "",
  ...
  const dynamicContext = [
    ...
    ...(opts.recentLocations?.trim()
      ? [
          ...
          'For "where am I now?": trust CURRENT POSITION and respect its stated age — an old timestamp on a phone that publishes only on movement still means they are most likely there, not that the data is wrong. The history list is past stays, NOT where they are now.',
          'CURRENT POSITION carries an age ("as of N min ago"). When that age is more than an hour, it is a data gap: if they ask for recent movement or a timeline up to now, say plainly that your location record stops there ("the last point I have is ~N min ago, so I may be missing anything after that") instead of guessing what happened in the gap. The same holds for a hole BETWEEN stays (a stretch with no logged stay): note it plainly as "no location logged for that stretch" and move on — do not backfill it with a guessed activity ("probably in transit") and do not reassure them the data is fine. For anything time-sensitive, call get_recent_locations for the freshest read.',
          "=== END LOCATION ===",
        ]
      : []),
